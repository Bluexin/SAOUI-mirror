...
public class SAOEntityPiecesFX extends EntityFX {
    ...
    private void renderQueued(Tessellator tessellator) {
        ...
        tessellator.getWorldRenderer().setColorRGBA_F(this.particleRed * colorIntensity, this.particleGreen * colorIntensity, this.particleBlue * colorIntensity, 1.0F);
        ...
        if (a < Math.PI) {
            tessellator.getWorldRenderer().addVertexWithUV(xPos + x1 * cos, yPos + y1, zPos + z1 * sin, 0D, 1D);
            tessellator.getWorldRenderer().addVertexWithUV(xPos + x2 * cos, yPos + y2, zPos + z2 * sin, 1D, 1D);
            tessellator.getWorldRenderer().addVertexWithUV(xPos + x3 * cos, yPos + y3, zPos + z3 * sin, 1D, 0D);
            tessellator.getWorldRenderer().addVertexWithUV(xPos + x4 * cos, yPos + y4, zPos + z4 * sin, 0D, 0D);
        } else {
            tessellator.getWorldRenderer().addVertexWithUV(xPos - x1 * cos, yPos + y1, zPos - z1 * sin, 0D, 0D);
            tessellator.getWorldRenderer().addVertexWithUV(xPos - x2 * cos, yPos + y2, zPos - z2 * sin, 1D, 0D);
            tessellator.getWorldRenderer().addVertexWithUV(xPos - x3 * cos, yPos + y3, zPos - z3 * sin, 1D, 1D);
            tessellator.getWorldRenderer().addVertexWithUV(xPos - x4 * cos, yPos + y4, zPos - z4 * sin, 0D, 1D);
        }
